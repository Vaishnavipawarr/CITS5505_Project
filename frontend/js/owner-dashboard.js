...
const ownerRest =
  DUMMY.restaurants.find((r) => r.id === ownerRestId) || DUMMY.restaurants[0];
    ...
    .map(
      ...
      <div class="orev ${r.ownerReply ? "has-reply" : "needs-reply"}" id="orev-${r.id}">
        <div class="d-flex align-items-center gap-3 mb-2">
          ...
          <div class="flex-1">
            <div class="reviewer-name">${escapeHtml(r.customerName)}</div>
        ...
        <div class="reply-compose ${r.ownerReply ? "" : "open"}" id="compose-${r.id}">
          <textarea id="replyInput-${r.id}" placeholder="Write a professional, helpful reply to this review…">${escapeHtml(r.ownerReply || "")}</textarea>
...
function submitReply(id) {
  ...
  const reviews = getReviews().map((r) =>
    r.id === id ? { ...r, ownerReply: text, ownerReplyDate: "Just now" } : r,
  );
  saveReviews(reviews);
  toast("Reply posted!", "✓");
  loadKPIs();
