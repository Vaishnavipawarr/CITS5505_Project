...
window.appReady.then(() => {
  ...
  applyInitialSearchFromUrl();// restaurants.js — Loads restaurants from API, filters, builds card grid

// Current logged-in user object (null if not logged in)
let currentUser = null;
...
// Fallback images for each restaurant by ID
...
// Fallback descriptions for each restaurant by ID
...
// Returns the correct review/dashboard link for a restaurant based on user role
...
// Returns the correct button label for a restaurant card based on user role
...
// Fetches restaurants and renders filtered cards into the grid
async function renderCards() {
  ...
  // Filter by search term, cuisine type and price range
...
// Reads the ?q= param from the URL and pre-fills the search input
...
// Re-render cards whenever search or filter inputs change
const filterInputs = [
