...
if (user)
  document.getElementById("navRight").innerHTML =
    `<a href="${user.role === "customer" ? "customer-dashboard.html" : "owner-dashboard.html"}" class="btn btn-amber btn-sm">Dashboard</a>`;

// Populate rest filter
const fr = document.getElementById("filterRest");
DUMMY.restaurants.forEach(
  (r) => (fr.innerHTML += `<option value="${r.id}">${r.name}</option>`),
);
...
function renderRevs() {
  ...
  if (restFilt !== "all")
    reviews = reviews.filter((r) => r.restaurantId === restFilt);
    ...
    .map(
      ...
      <div class="rev-card">
        <div class="d-flex align-items-center gap-3 mb-3">
          ...
          <div class="flex-1">
            <div style="font-weight:600;font-size:.9rem;color:var(--cream)">${escapeHtml(r.customerName)}</div>
