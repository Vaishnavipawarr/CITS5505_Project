...
function renderMyReviews() {
  ...
  document.getElementById("replyCount").textContent = reviews.filter(
    (r) => r.ownerReply,
  ).length;
    ...
    .map(
      ...
      <div class="rev-card">
        <div class="rev-rest-row">
          ...
          <div>
            <div class="rev-rest-name">${escapeHtml(r.restaurantName)}</div>
        ...
        ${r.tags && r.tags.length ? `<div class="rev-chips">${r.tags.map((t) => `<span class="rev-chip">${escapeHtml(t)}</span>`).join("")}</div>` : ""}
        ${r.ownerReply ? `<div class="owner-reply"><div class="lbl">✦ Owner's Reply · ${escapeHtml(r.ownerReplyDate || "")}</div><p>${escapeHtml(r.ownerReply)}</p></div>` : ""}
      </div>
    `,
    )
    .join("");
...
function renderBrowse() {
  ...
  if (search)
    reviews = reviews.filter(
      (r) =>
        r.restaurantName.toLowerCase().includes(search) ||
  ...
  if (restFilter !== "all")
    reviews = reviews.filter((r) => r.restaurantId === restFilter);
    ...
    .map(
      ...
      <div class="pub-rev">
        <div class="d-flex align-items-center gap-3 mb-3">
          ...
          <div class="flex-1">
            <div style="font-weight:600;font-size:.88rem;color:var(--cream)">${escapeHtml(r.customerName)}</div>
            <div style="font-size:.72rem;color:var(--muted)">Reviewed ${escapeHtml(r.restaurantName)} · ${escapeHtml(r.date)}</div>
          </div>
        ...
        <p style="font-size:.86rem;line-height:1.72;color:var(--text);margin-bottom:.75rem;">${escapeHtml(r.text)}</p>
        ${r.tags && r.tags.length ? `<div class="rev-chips">${r.tags.map((t) => `<span class="rev-chip">${escapeHtml(t)}</span>`).join("")}</div>` : ""}
        ${r.ownerReply ? `<div class="owner-reply"><div class="lbl">✦ Owner's Reply</div><p>${escapeHtml(r.ownerReply)}</p></div>` : ""}
      </div>
    `,
    )
    .join("");
...
function submitReview() {
  ...
  if (editId) {
    ...
    const newRev = {
      ...
      customerAvatar: user.avatar || "https://i.pravatar.cc/80?img=47",
      restaurantId: restId,
      restaurantName: rest.name,
      restaurantImg: rest.img,
      rating,
